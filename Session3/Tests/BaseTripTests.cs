...
namespace Tests
{
    ...
    public class BaseTripTests : BaseWebServiceTest
    {
        ...
        /// <summary>
        /// override the WebServiceDriver and add the key “Auth” and value “AuthKey” to the deta request header
        /// </summary>
        [TestMethod]
        public void OverrideWebDriver()
        {
            this.ManagerStore.AddOrOverride(new WebServiceDriverManager(() =>
            {
                HttpClient client = HttpClientFactory.GetClient(
                    new Uri(Config.GetValueForSection(ConfigSection.WebServiceMaqs, "WebServiceTripUri")), WebServiceConfig.GetWebServiceTimeout());
                client.DefaultRequestHeaders.Authorization =
                    new AuthenticationHeaderValue("Auth", Config.GetValueForSection(ConfigSection.WebServiceMaqs, "AuthKey"));
                return client;
            }, this.TestObject));
        } 
